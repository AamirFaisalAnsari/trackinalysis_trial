...
if ~isempty(ntracks)
    ...
    for track = 1:ntracks
        ...
        [par(track,:)] = track_fit_function(track_msd,alpha(track),clip_factor,figure_on);
    end
...
function [par] = track_fit_function(msd,alpha,clip_factor,figure_on)
% clear all
% close all
% clc
% %collagen
...
msd_cell=msd_cell(~isnan(msd_cell));%this has to be ath the bottom
...
if sum(msd_cell) == 0 || isnan(sum(msd_cell))==1 %|| isnan(alpha)
    ...
    if alpha<0.95
        ...
        r2=confined_walk_plot(P,tau_cell,msd_cell,sem_cell,figure_on);
        par1=sqrt(P(1));
        par2=P(2);
        par3=P(3);
    ...
    elseif alpha>1.05
        ...
        r2=convective_randwalk_plot(input_convec,tau_cell,msd_cell,sem_cell,figure_on);
        par1=input_convec(1);
        par2=input_convec(2);
        par3=nan;
    ...
    r2=randwalk_plot(D,tau_cell,msd_cell,sem_cell,figure_on);
    par1=D;
    par2=nan;
    par3=nan;
    ...
    r2=prandwalk_plot(P,tau_cell,msd_cell,sem_cell,figure_on);
    par1=P(1);
    par2=P(2);
    par3=nan;
    if exitflag~=1 %|| par1<0
...
function r2=confined_walk_plot(P,t,msd,sem,figure_on)
%%%eq=msd_sat*(1-par1*exp(-4*confined_D*t/msd_sat))
predicted=P(1).*(1-P(2).*(exp(-4*P(3).*t./P(1))));
...
function r2=convective_randwalk_plot(input_convec,t,msd,sem,figure_on)
convective_v=input_convec(1);
D=input_convec(2);
...
function r2=randwalk_plot(D,t,msd,sem,figure_on)
predicted=4*D*t;
...
function r2=prandwalk_plot(P,t,msd,sem,figure_on)
predicted=2*P(1)^2*P(2).*(t-P(2).*(1-exp(-t./P(2))));
% close all
