...
waithandle = waitbar(0,sprintf('Reading Image File...\nIf this takes a long time, consider cropping to ROI.\nReading frame %d',0));
while  ifd_pos ~= 0 

    img_indx = img_indx + 1;
    if max(indices)>1000
        waitbar(img_indx/1000,waithandle,sprintf('Reading Image File...\nIf this takes a long time, consider cropping to ROI.\nReading frame %d...',img_indx));
    else
        waitbar(img_indx/max(indices),waithandle,sprintf('Reading Image File...\nIf this takes a long time, consider cropping to ROI.\nReading frame %d...',img_indx));
    end
    clear IMG;
...
%clean-up
waitbar(1,waithandle,sprintf('Reading Image File.......Complete'));pause(0.5);
close(waithandle);
