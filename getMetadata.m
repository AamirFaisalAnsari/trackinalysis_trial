function [slope,name_keys,values_all_keys]=getMetadata(dataFile)
% clear all;
% clc;
% close all;
...
for i = 1%:length(files)
    ...
    values_all_keys = data.Var2(contains(data.Var1, KeyToExtract));
    values_all_keys_rearranged = data.Var1(contains(data.Var1, KeyToExtract));
    name_keys=values_all_keys_rearranged;
    values_time_stamp = data.Var2(contains(data.Var1, 'TimeStamp'));
    %numericValues = cellfun(@str2double, values, 'UniformOutput', false);
    numericValues=(str2double(values_time_stamp));
