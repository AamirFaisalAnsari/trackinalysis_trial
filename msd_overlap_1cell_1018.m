...
cd 'S:\JayHou\exp1024\migration';

pix=330;%nm%10X = 652, 20X = 330
dt = 5; %min
...
for i=1:numel(file_cell)
    ...
    for j=1:numel(files)
        [path name ext]=fileparts([char(file_cell(i)) '\' char(files(j).name)]);
...
maxa=0;
for i=1:numel(file_cell2)
    data=load(char(file_cell2(i)));
    [frameno b]=size(data);
...
figure; plot([15:15:15*length(mean_msd)], mean_msd, 'b'); 
hold on; plot([15:15:15*length(mean_msd)], 4*[15:15:15*length(mean_msd)]*mean(rmc), 'r');
FileName = strcat (Filepath, 'msd_overlap_1cell.mat');
save(FileName)
