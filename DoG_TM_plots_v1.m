...
%sheet_name={'Control','20 ug IM7','100 ug IM7'};
%sheet_name={'Control','10 uM Nocodazole','1000 uM Nocodazole'};
sheet_name={'Main'};

%sheet_name_print={'','Control','20 ug/ml IM7','100 ug/ml IM7',''};
%sheet_name_print={'','Control','Treatment',''};
sheet_name_print={'','Control',''};
%sheet_name_print={'','Control','10 uM Nocodazole','1000 uM Nocodazole',''};


timestep=1;
Lpad=0.5;

%unit_conversion=1.5;%90s for 20 and 100 ug/ml IM7
unit_conversion=100/60;%98s for 1000 uM Nocodazole %103s for 10 uM Nocodazole
...
for sheet_no=1:1
    ...
    f=figure(2);
    f.Position = [100 100 540 900];
    ylabel_list={'Speed (\mum/min)','RMC (\mum^2/min)','Linearity Forw Prog','Alpha'};
    proper_indices=[5,11,6,7];
    upper_lim=[50,400,1,2];
    ...
    for i=1:4
        ...
        if sheet_no~=1
            if i==1
                Unit_conv_mult=60/unit_conversion;
            elseif i==2
                Unit_conv_mult=1/unit_conversion;
            else
                Unit_conv_mult=1;
            end
        end
        scatter(sheet_no+0*data_slow(:,proper_indices(i)),Unit_conv_mult*data_slow(:,proper_indices(i)),'mo','jitter','on','jitterAmount',0.15);
        
        plot(linspace(sheet_no-Lpad,sheet_no+Lpad,100),linspace(sheet_no-Lpad,sheet_no+Lpad,100)*0+Unit_conv_mult*nanmedian(data_slow(:,proper_indices(i))),'m-');
    ...
    for i=1:length(video_id_slow)
        ...
        temp_alpha=data_slow(i,7);
        
        if temp_alpha<0.95
            color='g';
        elseif temp_alpha<1.05
            color='r';
        elseif temp_alpha<10
            color='b';
        else
            color='k';
        end
        
        hi=plot(timestep*(1:length(temp_msd)),temp_msd','Color',color);
    end
    
    legend('');
    cols =  ["green", "red", "blue","black"];
    col_names = ["\alpha<0.95", "\alpha>0.95 & \alpha<1.05", "\alpha>1.05","Poor fit"];
...
%         % histogram(Mega_combine_1_signal(:,2))
%         % histogram(Mega_combine_1_noise(:,2))
%         scatter(data_slow(:,pre_pad+3),data_slow(:,i+pre_pad-1),'mo');
%
