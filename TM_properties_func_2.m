%function combining_all=TM_properties_func(filename,timestep)
%
...
if Total_no_filt_track==-0.5
    combining_all=zeros(2,(Total_no_frames+9));
else
    ...
    for i=1:Total_no_track
        ...
        if sum(ismember(filt_track_ID,temp_id))==1
            track_median_speed(count,:)=(conversion/timestep)*str2double(data.Children(2).Children(6).Children(2*i).Attributes(20).Value);
            count=count+1;
        end
    end
    ...
    [alpha,~,r2fit] = fitLogLogMSD_afa(MSD(:,:,2),timestep);
...
%     box on;
    ...
    %hold off;
    %% track properties
    Track_properties=[RMC',track_median_speed,rand_speed',pers',alpha_quality,mean_spot_area',mean_spot_aspectratio',total_no_spots_in_filt_track'];
