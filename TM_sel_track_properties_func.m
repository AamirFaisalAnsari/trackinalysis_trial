function [combining_all,median_no_spots_in_frame]=TM_sel_track_properties_func(filename,video_id,timestep,clip_factor,sel_id)
...
% filename= 'H:\Aamir experiment data\PA Gels\Human T cells on 2D gels\All HT on fibronectin\\03242023_NoCART_10kP_20x_089_1_Slow_Model.xml';
% 
% sel_id=[22,6,5,32];
% 
...
if Total_no_filt_track==-0.5 %|| median_no_spots_in_frame<40
    ...
    for i=1:Total_no_filt_track
        aac=data.Children(2).Children(8).Children(2*i).Attributes.Value;
        if ismember(str2num(aac),sel_id)
            filt_track_ID{filt_count,:}=aac;
            filt_count=filt_count+1;
        end
    end
    
    Total_no_filt_track=size(filt_track_ID,1);

    %filt_track_ID{ismember(str2num(filt_track_ID{:,:}),sel_id).',:}=[];
...
%         box on;
    ...
    Track_properties=[video_id,total_no_spots_in_filt_track',med_spot_area',med_spot_aspectratio',track_med_speed,m_linearity_forward,alpha_quality,par];
    
    %Just Msd
    just_msd=transpose(MSD(:,:,2));
    
    %combining_all=[Track_properties,nan(Total_no_filt_track,1),just_msd];
    combining_all=[Track_properties,nan(Total_no_filt_track,1)];
