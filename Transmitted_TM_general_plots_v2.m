...
filename5= [common_folder,'tie_20dec23_mtcells_glass_2x_IM7_50Pa09_Slow_Model_all.xlsx'];
...
all_filenames={{filename3};{filename4};{filename5}};
...
file_x_axis_label={'IM7 2X 50 Pa','IM7 2X 700 Pa','IM7 2X Glass'};
...
%     end
    ...
    criteriaAndColumns = {
        'RMC', @(x) (x >= 0 && x < inf),
        'speed', @(x) (x >= 0 && x < inf),
        'confinement', @(x) (x >= 0 && x < 1),
        'alpha', @(x) (x >= 0 && x <= 3),
        'no_of_spot', @(x) (x >= 1 && x <= inf),
...
%{'video_id','no_of_spot','area','aspect_ratio','speed','confinement','alpha','sat_MSD','tau_c','conf_D','RMC','directed_v','directed_D'}
%%%plot 1
%header_name={'speed','confinement','alpha','RMC'};
%header_name={'video_id','no_of_spot','area','aspect_ratio','speed','confinement','alpha','sat_MSD','tau_c','conf_D','RMC','directed_v','directed_D'};
header_name={'area','aspect_ratio','alpha','RMC'};


plot_these_data=header_name;
%plot_these_data_lables={'Speed (µm/min)','Linearity','\alpha','RMC (µm²/min)'};
%plot_these_data_lables={'video_id','no_of_spot','area','aspect_ratio','speed','confinement','alpha','sat_MSD','tau_c','conf_D','RMC','directed_v','directed_D'};
plot_these_data_lables={'area','aspect_ratio','alpha','RMC'};
...
for i = 1:length(plot_these_data)
    ...
    if strcmp(columnName,'aspect_ratio')==1
        %ax.YScale = 'log';
        %ax.YTick=[0.0001,0.1,1,10,100,1000];
        ylim([1,2.5]);
    end
    
    if strcmp(columnName,'area')==1
        %ax.YScale = 'log';
        %ax.YTick=[0.0001,0.1,1,10,100,1000];
        ylim([0,400]);
    end
...
% 
% %%%plot meta data
% header_name={'speed','no_of_spot','RMC','confinement','aspect_ratio','area',''};
% plot_these_data=header_name;
